...
class Zero123(nn.Module):
    def __init__(self, device, fp16=True, t_range=[0.02, 0.98], model_key="ashawkey/zero123-xl-diffusers", image = None):
        super().__init__()
        self.image = image
        self.device = device
        self.fp16 = fp16
        ...
        # self.image_torch = torch.from_numpy(image).permute(2, 0, 1).unsqueeze(0).to(self.device)
        # self.image_torch = F.interpolate(self.image_torch, (self.opt.ref_size, self.opt.ref_size), mode="bilinear", align_corners=False)
        ...
        #In diffusion models, the UNet is used to predict the noise component at each step of the denoising process:
    ...
    def make_image_torch(self, ref_size =256):
        x = torch.from_numpy(self.image).permute(2, 0, 1).unsqueeze(0).to(self.device)
        x = F.interpolate(x, (ref_size, ref_size), mode="bilinear", align_corners=False)
    ...
    def refine(self, pred_rgb, elevation, azimuth, radius, 
        ...
        for i, t in enumerate(self.scheduler.timesteps[init_step:]):
            ...
            #The UNet’s output, noise_pred, represents the predicted noise for the current timestep.
        ...
        return imgs
    #guidance_zero123.train_step(images, vers, hors, radii, step_ratio=step_ratio if self.opt.anneal_timestep else None, default_elevation=self.opt.elevation)
    def train_step(self, pred_rgb, elevation, azimuth, radius, step_ratio=None, guidance_scale=5, as_latent=False, default_elevation=0):
        ...
        with torch.no_grad():
            ...
            self.make_image_torch()
            # print("embedding", self.embeddings)
            # print("self", self.image_torch)
            # # print("time")
