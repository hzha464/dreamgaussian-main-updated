...
class GUI:
    ...
    def prepare_train(self):
        ...
        # print("opt", self.opt.lambda_zero123)
        ...
        if self.guidance_zero123 is None and self.enable_zero123:
            ...
            if self.opt.stable_zero123:
                ...
                self.guidance_zero123 = Zero123(self.device, model_key='ashawkey/zero123-xl-diffusers', image=self.input_img)
        ...
        #Purpose: These steps are meant to convert the input image and mask into a format suitable for use with PyTorch, with the correct dimensions, batch size, and device.
        #Input Image and Mask: Both are converted to tensors and resized to match a specific reference size, ensuring they are consistent during training.
        #Interpolation Method: Bilinear interpolation is used to resize the images smoothly.
    ...
    def train_step(self):
        ...
        for _ in range(self.train_steps):
            ...
            # print(images)
            # print(hor, ver)
            kiui.vis.plot_image = plot_image
            image_jepg = kiui.vis.plot_image(images, save = True)
            # print(type(images))
            # guidance loss
            image_png = image_jepg.astype(np.float32)
            pil_image = transforms.ToPILImage()(image_png)

            labels_for_classification =  ["thin","fat"]
            scores = self.classifier(pil_image,
                    candidate_labels = labels_for_classification)
            for param in self.classifier.model.parameters():
                param.requires_grad = False
            print(scores)
            plt.imshow(image_jepg.astype(np.float32))
            plt.show()
                


            # print("loss" ,self.opt.lambda_zero123 * self.guidance_zero123.train_step(images, vers, hors, radii, step_ratio=step_ratio if self.opt.anneal_timestep else None, default_elevation=self.opt.elevation))
            # print(scores)
            # print(scores[1]["label"], scores[1]["score"])
            ...
            if self.enable_zero123:
                ...
                print("pre-loss", loss)
                loss = (loss + (100 - scores[1]["score"] * 100))/2
                # print("score", (100 - scores[1]["score"] * 100))
                print("after_added", loss)
